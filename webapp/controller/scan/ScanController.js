sap.ui.define([
	...
	return {
		 ...
		 */
		getScanById: function(iScanId, aScans) {
			//Get the selected scan from the array of all scans according to the id.
			for (var i = 0; i < aScans.length; i++) {
    			var oTempScan = aScans[i];
		 ...
		 */
		getInstrumentById: function(iInstrumentId, aQuotations) {
			//Get the selected instrument from the array of all quotations according to the id.
			for (var i = 0; i < aQuotations.length; i++) {
    			var oTempQuotation = aQuotations[i];
		...
		getScanForWebService: function(oScan) {
			...
			oScanWs.setProperty("/name", oScan.name);
			oScanWs.setProperty("/description", oScan.description);
			oScanWs.setProperty("/lastScan", oScan.lastScan);
			oScanWs.setProperty("/executionStatus", oScan.executionStatus);
			oScanWs.setProperty("/completionStatus", oScan.completionStatus);
